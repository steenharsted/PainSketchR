...
#' into a valid pain drawing data structure -- see [pd_check_data] for more details about 
#' _valid pain drawing data structure_
#' 
#' @details
#' 
...
#'
#' @returns A pain drawing data structure -- see [pd_check_data] for more details.
#'
#' @export
#' @examples
#' pd_json2pd(pd_demo_filepath_json_file_1) # Load data
#' pd_json2pd(pd_demo_filepath_json_file_1) |> pd_recreate_drawing() # Plot data
#' pd_json2pd(c(pd_demo_filepath_json_file_1, pd_demo_filepath_json_file_2))|> pd_recreate_drawing() # Plot data in two facets#' 
#' 
pd_json2pd <- function(files) {
  ...
  if(!is.character(files)) {return(data.frame())}
  ...
  if(!file.exists(files)) {return(data.frame())}
