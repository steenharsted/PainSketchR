...
#' @export
#' @examples
#' pdr_poly_anatomy_overlap(pdr_demo_data, p, pdr_demo_anatomy,  "Back_right_calf")
#'
#'
#'
pdr_poly_anatomy_overlap <- function(
  p,
  anatomy_pd,
  anatomy_id = "Not specified"
) {
  ...
  if (
    !tibble::is_tibble(anatomy_pd) || !all(c("x", "y") %in% names(anatomy_pd))
  ) {
      ...
      if (!tibble::is_tibble(df)) {
        ...
        df |>
          ...
          dplyr::group_map(\(dfgr, i_dfgr) {
            ...
            pdr_poly_clip(dfgr, anatomy_pd, op = "intersection") |>
              dplyr::select(-i)
