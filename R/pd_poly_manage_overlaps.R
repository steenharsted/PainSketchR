...
merge_polygons <- function(df, method="merge") {
  if(any({df |> dplyr::group_by(i) |> unique() |> count() |> pull(n) } < 3)) {
    stop("Can not merge strokes/polygons which have no area (points and lines). Please use `pd_poly_cleanup()` to buffer points and lines.")
  }

  # df is a data frame of i,x,y columns - i identifies each polygon
  # the sf package uses some very specific data structures, so we need 
  # some heavy handed wrangling:
  polys <- df |>
    dplyr::group_by(i) |> 
    dplyr::group_modify(\(p, indx) {close_poly(p)}) |> # sf package expects polygons to be closed
    tidyr::nest() |> # Now split the data frame by grouping variable into list elements
    dplyr::pull(data) |> # 'data' is a new col created by nest function -- it holds non-grouping variables (x,y)
    purrr::map(\(a_tib) {as.matrix(a_tib, ncol=2, byrow=FALSE)}) |> # 'data' col is list of tibbles 
...
# split into separate polygons again
  parts <- sf::st_cast(u, "POLYGON")

# back to tibble structure
  result <- parts |>
    sf::st_coordinates() |>
    tibble::as_tibble() |>
