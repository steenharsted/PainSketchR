...
#'   (`png[,, 4]`) must contain alpha values in the range \[0, 1\].
#'   Typically the `.png` list-column produced by [pd_to_png()].
...
#'
#' @returns A numeric vector of length `length(pngs)`. Each element is the
...
#'   dplyr::mutate(.png = pd_to_png())
#'
#' # Mean intensity across all drawn pixels
#' pd_alpha_intensity(pd$.png)
#'
#' # Exclude very faint pixels (alpha <= 0.05)
#' pd_alpha_intensity(pd$.png, alpha_range = c(0.05, 1))
#'
#' # Use inside mutate()
#' pd |> dplyr::mutate(intensity = pd_alpha_intensity(.png))
#'
pd_alpha_intensity <- function(pngs, alpha_range = c(0, 1)) {
  # --- input checks ---
  if (!is.list(pngs)) {
    stop(
  ...
  purrr::map_dbl(pngs, function(png) {
...
#' pd_alpha_area(pd$.png)
#'
#' # Proportion of heavily drawn pixels only (alpha > 0.5)
#' pd_alpha_area(pd$.png, alpha_range = c(0.5, 1))
#'
#' # Use inside mutate()
#' pd |> dplyr::mutate(
#'   area         = pd_alpha_area(.png),
#'   area_over_50 = pd_alpha_area(.png, alpha_range = c(0.5, 1))
#' )
pd_alpha_area <- function(pngs, alpha_range = c(0, 1)) {
  # --- input checks ---
  if (!is.list(pngs)) {
    stop(
      "`pngs` must be a list of numeric arrays with dimensions [height, width, 4]."
