...
#' @export
#' @examples
#' pdr_anatomy_merge(pdr_demo_anatomy)
#'
#' bi <- system.file("extdata", "mird_body_background.png", package = "paindrawings")
#' pdr_demo_anatomy |> pdr_recreate_drawing(background_image = bi) # Note: execution time > 5 sec
#' pdr_demo_anatomy |> pdr_anatomy_merge() |> pdr_recreate_drawing(background_image = bi)
#'
#' pdr_demo_anatomy |> dplyr::filter(id %in% c("Front_left_thigh", "Front_left_leg", "Front_left_foot")) |> pdr_recreate_drawing(background_image = bi)#'
#' pdr_demo_anatomy |> dplyr::filter(id %in% c("Front_left_thigh", "Front_left_leg", "Front_left_foot")) |> pdr_anatomy_merge() |> pdr_recreate_drawing(background_image = bi)
#'
pdr_anatomy_merge <- function(pd) {
  if (!pdr_check_data(pd, verbose = FALSE)) {
    stop("`pd` must be a valid pain drawing data structure")
  }
  ...
  just_p_collapsed <- tibble::tibble(
    p = pd |> dplyr::pull(p) |> purrr::list_rbind(names_to = 'i') |> list()
  )
  # Now merge overlapping strokes/polygons ... this is time consuming!
  just_p_collapsed <- just_p_collapsed |>
    mutate(p = pdr_poly_manage_overlaps(p, method = "union"))
