...
#'
#' Extracts metadata or variables from a pain drawing (`pdr`) list-column.
...
#'
#' @param pdr A valid pain drawing object (typically a list-column) as checked
#'   by [pdr_check_data()].
...
#'
#' @return
#' If `var[1]` refers to a regular element, a vector of length `length(pdr)` is
#' returned.
#'
#' If `var[1]` is `".strokes"` or `".points"`, a list of length `length(pdr)`
