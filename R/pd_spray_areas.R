#' Calculate are of pain drawing created with spraying
#'
...
pd_spray_areas <- function(png_files, alpha_multiply=TRUE) {
  if(!is.character(png_files)) { stop("One of more png file paths (character) must be specified") }
  
  result <- tibble::tibble(file=png_files, R=NA, G=NA, B=NA)

  for (i in seq_along(png_files)) {
    if(fs::is_file(png_files[i])) {
      png_data <- tryCatch(expr = {png::readPNG(png_files[i])}, error = \(e) {NA})
