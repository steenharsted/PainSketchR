draw_my_group_pain <- function(
  ...
  if (VAR1 %>% unique() %>% length() < n_groups[1]) {
    stop(paste0(
      "You have asked to split the variable '",
      variables_to_plot[1],
      "' into ",
      n_groups[1],
      " groups. There is only ",
      VAR1 %>% unique() %>% length(),
      " unique values present in '",
      variables_to_plot[1],
      "'. Please reduce the n_groups value."
    ))
  }
  ...
  if (
    ...
    message(paste0(
      ...
      variables_to_plot[1],
      "' is a ",
      VAR1 %>% vctrs::vec_ptype_abbr(),
      ". I cannot collapse this type of variable into fewer groups. Continuing with ",
      VAR1 %>% unique() %>% length(),
      " groups."
    ))
    n_groups[1] <- VAR1 %>%
  ...
  if (same_n_in_groups) {
    ...
    message(paste0(
      "same_n_in_groups is set to TRUE. The smallest group is ",
      min_group
    ))
    message(paste0(
      "max_n_in_groups is to ",
      max_n_in_groups,
      ". Reducing all groups to match ",
      min(min_group, max_n_in_groups)
    ))

    if (
      txt != "pct" &
        (VAR1 %>% vctrs::vec_ptype_abbr() == "dbl" |
          VAR2 %>% vctrs::vec_ptype_abbr() == "dbl")
    ) {
      warning(
  ...
  pd_df_and_coord_flipped <- paindrawing_coord_object %>% # Insert paindrawing coordinate object here
    ...
    full_join(pd_df, multiple = "all", relationship = "many-to-many")
  ...
  low <- plot_pct_flipped_reduced_data$pct %>%
    quantile(0.33, n.rm = TRUE, names = FALSE)
  high <- plot_pct_flipped_reduced_data$pct %>%
    quantile(0.66, n.rm = TRUE, names = FALSE)
  ...
  plot_pct_flipped_reduced <- plot_pct_flipped_reduced_data %>%
    ...
    geom_jitter(
      aes(alpha = if_else((pct^2) / 2 < min_alpha, min_alpha, (pct^2) / 2)),
